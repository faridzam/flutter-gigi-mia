...
import '../models/PostTestModel.dart';

class PostTestHelper{
  static Database? _db;

  static const String DB_Name = 'pengasuh_gigi.db';
  static const String Table = 'posttest';
  ...
  Future<int> insertPostTest(PostTestModel postTest) async{
    var dbClient = await db;
    var res = await dbClient!.insert(Table, postTest.toMap());
    return res;
  }

  // A method that retrieves all the breeds from the breeds table.
  Future<List<PostTestModel>> getPostTest() async {
    ...
    return List.generate(maps.length, (index) => PostTestModel.fromMap(maps[index]));
  }

  Future<PostTestModel?> getScoreUser() async {
    var dbclient = await db;
